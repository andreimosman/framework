<?
	...
	class MBoleto extends MBanco {
		 ...
		 * com 10 digitos p/ convenio + 10 digitos p/ id (seu numero) + 
		 * 2 dígitos p/ carteira e (provavelmente) faz um PAD para o tamanho de 25 do campo.
		 */
		protected function obtemCampoLivre() {
			$campoLivre = $this->padZero($this->convenio,10) . $this->padZero($this->id,10) . $this->carteira;
			return($this->padZero($campoLivre,25));
		}
		
		
		protected function obtemNossoNumero() {
			//return( $this->padZero($this->convenio . ((float)$this->id),10) );
